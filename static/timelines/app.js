        ...
        if ( xInBounds && yInBounds) {
            ...
            if(confirm("Are you sure you want to delete " + eventData.name)){
                events = events.filter(
                    x => {
                        return !(shallowEqual(x, eventData))
                    }
                )
                change = true
                saveTimeline()
...
function saveTimeline() {
    ...
    $.ajax({
      ...
      success: function(response) {
        if (response === "Success") {
            window.showToast("Timeline saved successfully!", "Sucess", 1000)
            console.log("Timeline saved successfully!");
        } else {
            window.showToast("Failed to save timeline:", "warning", 1000)
...
function saveEvent(){
    ...
    if(!name){
        window.showToast("Name can't be empty", "warning", 1000)
        return
    }
